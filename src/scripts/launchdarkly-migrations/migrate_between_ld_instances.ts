...
function getFlagDataByIndexOrKey(index: number, key: string): Promise<unknown> {
  return getJson(`${flagsDir}/${index}.json`).then((data) => data ?? getJson(`${flagsDir}/${key}.json`));
}
...
for (const [viewIndex, flagkey] of flagList.entries()) {
  ...
  if (flag && typeof flag === "object" && flag !== null && "viewKeys" in flag && Array.isArray((flag as { viewKeys?: unknown }).viewKeys)) {
    (flag as { viewKeys: string[] }).viewKeys.forEach((viewKey: string) => allViewKeys.add(viewKey));
  }
}
...
async function processOneFlag(index: number, flagkey: string): Promise<FlagProcessResult> {
  ...
  const flag = await getFlagDataByIndexOrKey(index, flagkey) as Record<string, any> | null;
